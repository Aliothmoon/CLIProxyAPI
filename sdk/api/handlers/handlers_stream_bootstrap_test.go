...
type canceledChunkStreamExecutor struct{}

func (e *canceledChunkStreamExecutor) Identifier() string { return "codex" }

func (e *canceledChunkStreamExecutor) Execute(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (coreexecutor.Response, error) {
	return coreexecutor.Response{}, &coreauth.Error{Code: "not_implemented", Message: "Execute not implemented"}
}

func (e *canceledChunkStreamExecutor) ExecuteStream(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (*coreexecutor.StreamResult, error) {
	ch := make(chan coreexecutor.StreamChunk, 1)
	ch <- coreexecutor.StreamChunk{Err: context.Canceled}
	close(ch)
	return &coreexecutor.StreamResult{Chunks: ch}, nil
}

func (e *canceledChunkStreamExecutor) Refresh(ctx context.Context, auth *coreauth.Auth) (*coreauth.Auth, error) {
	return auth, nil
}

func (e *canceledChunkStreamExecutor) CountTokens(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (coreexecutor.Response, error) {
	return coreexecutor.Response{}, &coreauth.Error{Code: "not_implemented", Message: "CountTokens not implemented"}
}

func (e *canceledChunkStreamExecutor) HttpRequest(context.Context, *coreauth.Auth, *http.Request) (*http.Response, error) {
	return nil, &coreauth.Error{
		Code:       "not_implemented",
		Message:    "HttpRequest not implemented",
		HTTPStatus: http.StatusNotImplemented,
...
type canceledStartStreamExecutor struct{}

func (e *canceledStartStreamExecutor) Identifier() string { return "codex" }

func (e *canceledStartStreamExecutor) Execute(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (coreexecutor.Response, error) {
	return coreexecutor.Response{}, &coreauth.Error{Code: "not_implemented", Message: "Execute not implemented"}
}

func (e *canceledStartStreamExecutor) ExecuteStream(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (*coreexecutor.StreamResult, error) {
	return nil, context.Canceled
}

func (e *canceledStartStreamExecutor) Refresh(ctx context.Context, auth *coreauth.Auth) (*coreauth.Auth, error) {
	return auth, nil
}

func (e *canceledStartStreamExecutor) CountTokens(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (coreexecutor.Response, error) {
	return coreexecutor.Response{}, &coreauth.Error{Code: "not_implemented", Message: "CountTokens not implemented"}
}

func (e *canceledStartStreamExecutor) HttpRequest(context.Context, *coreauth.Auth, *http.Request) (*http.Response, error) {
	return nil, &coreauth.Error{
		Code:       "not_implemented",
		Message:    "HttpRequest not implemented",
		HTTPStatus: http.StatusNotImplemented,
...
func TestExecuteStreamWithAuthManager_ContextCanceledChunkReturns499(t *testing.T) {
	executor := &canceledChunkStreamExecutor{}
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(executor)

	auth := &coreauth.Auth{
		ID:       "auth-canceled-chunk",
		Provider: "codex",
		Status:   coreauth.StatusActive,
	...
	if len(gotPayload) != 0 {
		t.Fatalf("expected empty payload, got %q", string(gotPayload))
	}
	...
	for msg := range errChan {
		...
		gotStatus = msg.StatusCode
		gotErr = msg.Error != nil
	}
	if !gotErr {
		t.Fatalf("expected terminal stream error")
	}
	if gotStatus != statusClientClosedRequest {
		t.Fatalf("expected status %d, got %d", statusClientClosedRequest, gotStatus)
	}
}

func TestExecuteStreamWithAuthManager_ContextCanceledStartReturns499(t *testing.T) {
	executor := &canceledStartStreamExecutor{}
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(executor)

	auth := &coreauth.Auth{
		ID:       "auth-canceled-start",
		Provider: "codex",
		Status:   coreauth.StatusActive,
		Metadata: map[string]any{"email": "test@example.com"},
	}
	...
	dataChan, _, errChan := handler.ExecuteStreamWithAuthManager(context.Background(), "openai", "test-model", []byte(`{"model":"test-model"}`), "")
	if dataChan != nil {
		t.Fatalf("expected nil data channel when stream bootstrap fails")
	}
	if errChan == nil {
